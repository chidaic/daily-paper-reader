...
tldr: 虚拟世界面临用户多模态请求需不同AI后端与计算资源的架构挑战。本文提出基于SLM的Agent Orchestration Gateway，通过边缘部署的SLM对用户提示分类实现意图驱动路由，解耦客户端与异构AI后端。在虚拟博物馆测试床中，微调后的亚十亿参数模型在边缘硬件上成为低延迟可靠路由器。该架构使虚拟代理成为分布式生成式AI服务的入口，提升了可扩展性与边缘支持能力。
source: arxiv
...
motivation: 虚拟世界需处理多模态请求，但直接嵌入不同AI后端降低可扩展性且维护复杂，亟需解耦的协调机制。
method: 提出基于SLM的Agent Orchestration Gateway，由边缘SLM分类用户意图，通过服务注册表路由到对应AI后端，透明调用。
result: 微调后的紧凑SLM在边缘硬件上作为可靠意图路由器，且分层配置（小模型路由+大模型生成）比单一模型更高效。
conclusion: SLM可支撑虚拟世界的AI服务编排，贡献了可扩展、支持边缘的AI交互架构，使代理成为分布式服务的访问点。
---

## 摘要
随着生成式AI能力的扩展，AI驱动的虚拟世界面临日益增长的架构挑战。用户通过虚拟世界界面以多模态方式交互，但其请求需要根本不同的AI后端模型和计算资源。将这些能力直接嵌入虚拟世界系统会降低可扩展性、增加维护复杂性，并限制协调分布在边缘和云基础设施上的服务的能力。本文提出了一种基于SLM的智能体编排网关，这是一种轻量级运行时协调机制，通过意图驱动的服务路由将虚拟世界客户端与异构AI后端解耦。边缘部署的SLM对每个用户提示的语义意图进行分类，可配置的服务注册表验证并解析路由决策，然后透明地调用所选后端，从而无需修改客户端应用程序即可在虚拟世界中引入新的AI能力。该网关在InterwovenXR虚拟博物馆测试平台上实现并评估。评估表明，紧凑型SLM可以在边缘硬件上作为可靠的意图路由器，并且任务特定微调可以将参数不足十亿的模型转化为实用的低延迟路由器。一种分层配置，将微调后的十亿参数模型作为路由器，与用于对话响应生成的较大SLM相结合，被证明可以在中端边缘硬件上部署，并且比将两者职责委托给单个模型更高效。研究结果表明，SLM可以支持虚拟世界中实用的AI服务编排，并且该工作为可扩展、可扩展和边缘支持的AI交互提供了一个经过评估的架构，使虚拟智能体成为分布式生成式AI服务的访问点。
